...
type Settings = { cash_balance: number; min_trade_value: number };
...
function recommend(type: string, aboveTrend: boolean | null, momentum: number | null, compositeScore?: number | null): Rec {
  if (type === "asx-stock") {
    ...
    if (aboveTrend)               return { action: "Hold",               badge: "hold", reason: "Above trend — check signal board for full score" };
    return { action: "Consider selling", badge: "out", reason: "Below 200d trend and weak composite score" };
  }
  if (type === "asx-etf") {
...
export default async function PortfolioPage() {
  const [holdingRows, settingsRows] = await Promise.all([
    ...
    sql`SELECT cash_balance, min_trade_value FROM portfolio_settings WHERE id = 1` as unknown as Promise<Settings[]>,
  ]);

  const settings: Settings = settingsRows[0] ?? { cash_balance: 0, min_trade_value: 500 };

  const [priceRows, fundRows] = await Promise.all([
    sql`SELECT ticker, date::text, close FROM prices WHERE ticker = ANY(${[...TICKERS, BENCHMARK]}) ORDER BY ticker, date ASC` as unknown as Promise<{ ticker: string; date: string; close: number }[]>,
  ...
  const allSignals = computeSignals(stockBars, fundsByTicker);
  const signalMap  = new Map(allSignals.map((s) => [s.ticker, s]));
  const { picks }  = rankSignals(allSignals, regime, 0.15);

  const tradePicks: TradePick[] = picks.map((p) => ({
    ticker:         p.ticker,
    label:          p.ticker.replace(".AX", ""),
    lastClose:      p.lastClose,
    compositeScore: p.compositeScore,
    sector:         p.fundamentals?.sector ?? null,
    company_name:   p.fundamentals?.company_name ?? null,
  ...
  const investedValue = rows.reduce((sum, r) => sum + (r.value ?? 0), 0);
  const totalValue    = investedValue + settings.cash_balance;
  const COLORS = ["#d4a24c", "#6fae8c", "#7b9fc4", "#c47b9f", "#a4c47b", "#c47ba4"];
  const allocationRows = [
    ...rows.map((r, i) => ({ label: r.label, value: r.value ?? 0, color: COLORS[i % COLORS.length] })),
    { label: "Cash", value: settings.cash_balance, color: "#4a5568" },
  ];
  ...
  return (
    <main>
      <div className="page">
              ...
              <div className="stat-grid">
                ...
                <div>
                  ...
                  <div className="stat-value positive" style={{ fontSize: 18 }}>
                    {fmtAud(settings.cash_balance)}
                    {totalValue > 0 && (
                      <span style={{ fontSize: 12, color: "var(--text-dim)", marginLeft: 6 }}>
                        {((settings.cash_balance / totalValue) * 100).toFixed(1)}%
                      </span>
                    )}
                  </div>
                </div>
            ...
            <div className="panel">
              ...
              <div style={{ display: "flex", height: 28, borderRadius: 4, overflow: "hidden", gap: 2 }}>
                {allocationRows.map((r) => {
                  ...
                  return (
                    <div key={r.label} style={{ width: `${pct}%`, background: r.color, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 600, color: "#11151c", overflow: "hidden", whiteSpace: "nowrap", padding: "0 6px" }} title={`${r.label}: ${pct.toFixed(1)}%`}>
        ...
        <TradeSuggestions
          picks={tradePicks}
          initialCash={settings.cash_balance}
          initialMinTrade={settings.min_trade_value}
        />
